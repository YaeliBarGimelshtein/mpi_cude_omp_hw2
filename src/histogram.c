...
#pragma omp parallel for shared(private_hist, other_hist) reduction(+ : histogram)
    ...
    else
    {
        ...
        int extra_work_second = num_work_for_each % 2;
        num_work_for_each /= 2;
...
#pragma omp parallel for shared(thread_hist)
        ...
        //TAKE CARE OF RESIDUALS
        if(extra_work_second != 0)
        {
            int start_second = 2 * num_work_for_each;
            for (int i = start_second; i < start_second + extra_work_second; i++)
            {
                total_hist[work_arr_nums[i]]++;
            }
        }
